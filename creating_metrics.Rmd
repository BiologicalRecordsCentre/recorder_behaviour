...
clusterEvalQ(cl,{library(sp);library(BRCmap);library(adehabitatHR);library(raster);library(Deducer);library(BRCmap)})
```
...
temp <- cbind(rarity_preference[rarity_preference$n > 400, 'recorder'],
              round(rarity_preference[rarity_preference$n > 400, c('median_rarity', 'median_diff_rarity','stdev','n')], 2))
...
```{r top_examples, fig.height=9, fig.width=6}
...
We can explore the values in more detail when we calculate them for everyone with enought records. Note we only calculate these values for recorders with a total number of days equal or more to the highest number of days we want to predict

```{r species_accumulation_all}
par(mfrow = c(1, 1))
...
This correlation plot shows that - of hte cut offs I chose - 20 is the probably the best value. This is because it is the metric that is best correlated with all the other values chossen.
...
![The three measures of recorder's spatial behaviour are all independent. Here each column shows a change in one variable while the others are held constant](figures/spatial_variables.png)
...
- *Median Rarity Difference*: Species are ranked according to the number of records in the entire dataset, and scaled to 100. This variable is the difference between the median rank overall and the median rank for the recorder. Negative numbers mean the recorder preferntially records common species while positive number means the recorder records rarer species.
...
Drop the recorders with less that 10 active days

```{r subset_colinearity}
...
* Work with `NA`s. We may have quite a few `NA`s in our data since some of the metrics can only be estimated for a subset of recorders, we want a method that can deal with this.
